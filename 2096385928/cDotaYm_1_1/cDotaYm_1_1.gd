extends Chara
const Utils = globalData.infoDs["g_aDotaProcess"]
const Buffs = globalData.infoDs["g_BuffList"]

#覆盖的初始化
func _info():
	pass
#继承的初始化，技能描述在这里写，保留之前的技能描述
func _extInit():
	._extInit()#保留继承的处理
	chaName = "影魔 - 3级"
	attCoe.atkRan = 2
	attCoe.maxHp = 3
	attCoe.atk = 7
	attCoe.mgiAtk = 0
	attCoe.def = 1
	attCoe.mgiDef = 4
	lv = 3
	evos = [] #可以升级到的生物id
	atkEff = "atk_dao"
	addSkillTxt("[color=#FF0000][b]敏捷[/b]：这个单位的攻击速度会随着攻击力提升而提升\n")
	addSkillTxt("毁灭阴影：每3秒，对最近三个单位造成100%物理攻击的物理伤害。")
	addCdSkill("hmyy",3)
	addSkillTxt("魔王降临：全场敌方单位减少30点护甲")
	addSkillTxt("魂之挽歌：每8秒释放灵魂，对被触及的单位造成100%物理攻击的魔法伤害。")
	addCdSkill("hzwg",8)
	addSkillTxt("魂之挽歌：死亡时释放灵魂，被死灵触及的单位减少30%攻击速度，持续4秒，并受到150%物理攻击的魔法伤害")
#进入战斗初始化，事件连接在这里初始化

func _connect():
	._connect() #保留继承的处理

func _onBattleStart():
	addBuff(Buffs.Buff_Talnet.new(Buffs.AGILITY))
	var chas = getAllChas(1)
	for cha in chas:
		cha.addBuff(Dt_MWJL_ArmorDefuse.new())


func _castCdSkill(id):
	._castCdSkill(id)
	if id == "hmyy":
		var chas = getAllChas(1)
		chas.sort_custom(self, "sort")
		var i = 0
		for cha in chas:
			ex(cha)
			i += 1
			if i == 3:
				break
	
	if id == "hzwg":
		var Vector = [Vector2(0,200),Vector2(300,0),Vector2(700,200),Vector2(300,400)]
		for i in Vector:
			exSkillHzwg(i)
		

func sort(a,b):
	if cellRan(a.cell, cell) < cellRan(b.cell, cell):
		return true
	return false

func ex(cha):
	var eff = Utils.Effect_CreateAoe("hmyy",cha.position,Vector2(0,-50))
	hurtChara(cha, att.atk, Chara.HurtType.PHY, AtkType.SKILL)

func _onCharaDel(cha):
	if cha == self:
		var Vector = [Vector2(0,200),Vector2(300,0),Vector2(700,200),Vector2(300,400),Vector2(0,0),Vector2(0,400),Vector2(700,0),Vector2(700,400)]
		for i in Vector:
			exDeath(i)

func exDeath(Vector):
	var eff = Utils.Effect_CreateToPos("hzwg",position,Vector,400)
	eff.connect("onInCell",self,"effInCell")

func exSkillHzwg(Vector):
	var eff = Utils.Effect_CreateToPos("hzwg",position,Vector,400)
	eff.connect("onInCell",self,"effInCell_Skill")

func effInCell(cell):
	var cha = matCha(cell)
	if cha != null && cha.team != team :
		cha.addBuff(Buffs.Buff_AtkSpeedDefuse.new(0.3,4))
		hurtChara(cha, att.atk * 1.5, Chara.HurtType.MGI, AtkType.SKILL)

func effInCell_Skill(cell):
	var cha = matCha(cell)
	if cha != null && cha.team != team :
		hurtChara(cha, att.atk, Chara.HurtType.MGI, AtkType.SKILL)

class Dt_MWJL_ArmorDefuse extends Buff:
	func _init(lv=1):
		attInit()
		att.def -= 30
		effId = "p_liuXue"
		id = "Dt_MWJL_ArmorDefuse"
		isNegetive=true


