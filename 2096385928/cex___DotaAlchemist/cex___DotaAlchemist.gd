...
func _extInit():
	...
	addSkillTxt("贪魔的贪婪：场上敌方单位死亡时额外获得8金币并恢复5%最大生命值")
	addSkillTxt("酸性喷雾：每6秒向区域喷洒高压喷雾，造成100%攻击力的物理伤害并削弱80护甲和减少50%攻速，持续3秒")
	...
	addSkillTxt("[color=#00FF00]化合物炼制指南：\n化合物基础威力：400魔法伤害和2秒眩晕\n每多炼制一秒，化合物的伤害就会提升300，晕眩时间延长1秒\n[color=#FF0000]尽情破坏吧！\n")
	addSkillTxt("由于电脑玩家智商不足无法调配药物，我们准备了化合物原浆供它投掷：\n电脑玩家的化合物不需要调配时间即可扔出，造成800魔法伤害和持续2秒的晕眩效果")
...
func _castCdSkill(id):
	if id == "sxpw":
		...
		for i in chas:
			...
			i.addBuff(Buffs.Buff_ArmorDefuseValue.new(80,3))
			i.addBuff(Buffs.Buff_AtkSpeedDefuse.new(0.5,3))
...
func DrugThrow():
	...
	for i in enemys:
		if team == 1:			
			...
			hurtChara(i, 400+300*DrugPreTime, Chara.HurtType.MGI, Chara.AtkType.SKILL)
		else:
			i.addBuff(Buffs.Buff_Stun.new(2))
			hurtChara(i, 800, Chara.HurtType.MGI, Chara.AtkType.SKILL)
